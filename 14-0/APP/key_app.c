...
uint8_t lcd_para_index = 0;
uint32_t Key3_ticks;
uint8_t Key2_flag = 1;
uint32_t key4_ticks;
uint8_t key4_flag; // 0为解锁
...
void key_proc(void)
{
    ...
    if (key_down == 2)
    {
        ...
        else if (lcd_displaymode == 0)
        {
            if (Key2_flag == 1)
            {
                Key3_ticks = HAL_GetTick();
                lcd_data_flag ^= 1;
                Key2_flag = 0;
                recd[0]++;
            }
    ...
    if (key_down == 4)
    {
        ...
        {
            para[lcd_para_index]--;
            if (para[lcd_para_index] < 1)
            {
                para[lcd_para_index] = 10;
            }
        }
        else if (lcd_displaymode == 0)
        {
    ...
    {
    case 1:
        if (++lcd_displaymode == 3)
            lcd_displaymode = 0;
        if (lcd_displaymode == 2)
        {
            para_set[0] = para[0];
            para_set[1] = para[1];
        }
    ...
    case 3:
        ...
        {
            para[lcd_para_index]++;
            if (para[lcd_para_index] > 10)
            {
                para[lcd_para_index] = 1;
            }
        }
        break;
    }
