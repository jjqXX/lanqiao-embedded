...
uint32_t speed_tick = 0;// 速度更新计时器
uint8_t speed_valid = 0;// 速度更新标志
float last_speed = 0.0f;// 上一次的速度值
...
void ic_proc(void)
{
    ...
    v_value = (tim_ic_val * 2 *PI * para[0]) / (para[1] * 100.0f);

    pwm_value = __HAL_TIM_GET_COMPARE(&htim2,TIM_CHANNEL_2);

     if((int)last_speed != (int)speed_val)
    {
        speed_valid = 1;
        speed_tick = uwTick;
    }
    else if(speed_valid == 1 && (uwTick - speed_tick) > 2000)
    {
        if(speed_val > recd[pwm_output_mode + 1])
        {
            recd[pwm_output_mode + 1] = speed_val;
        }
        speed_valid = 0;
    }
