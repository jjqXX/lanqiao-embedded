...
uint32_t key4_ticks = 0;
uint8_t error_cnt = 0;;
uint32_t error_ticks = 0;
...
void key_proc(void)
{
    ...
    if (lcd_disp_mode == 0 && key_down <= 3 && key_down > 0)
    {
        if (password_show[key_down - 1] == '@')
        {
            password_show[key_down - 1] = '0';
        }
        else if (++password_show[key_down - 1] > '9')
        {
            password_show[key_down - 1] = '0';
        }
    }
    else if (lcd_disp_mode == 0 && key_down == 4)
    {
        if (password_show[0] == password_data[0] && password_show[1] == password_data[1] && password_show[2] == password_data[2])
        {
            lcd_disp_mode = 1;
            key4_ticks = HAL_GetTick();
        }
        else
        {
            error_cnt++;
    ...
    if (uwTick - key4_ticks >= 5000 && lcd_disp_mode == 1)
    {
        lcd_disp_mode = 0;
        key4_ticks = 0;
        for (uint8_t i = 0; i < 3; i++)
