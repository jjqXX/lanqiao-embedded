 ...
 */
void pwm_set_duty_PA6(float Duty)
{
    // 根据占空比计算捕获/比较寄存器的值
    TIM3->CCR1 = (TIM3->ARR + 1) * (Duty / 100.0f);
}

void pwm_set_duty_PA7(float Duty)
{
    // 根据占空比计算捕获/比较寄存器的值
 ...
 */
void pwm_set_frequency_PA6(int Frequency)
{
    // 获取定时器的时钟频率（假设TIM2使用的时钟频率为TIM2_CLK）
    uint32_t TIM3_CLK = 80000000;  // 例如72MHz, 需要根据实际情况调整
...
void pwm_set_frequency_PA7(int Frequency)
{
    // 获取定时器的时钟频率（假设TIM2使用的时钟频率为TIM2_CLK）
    uint32_t TIM3_CLK = 80000000;  // 例如72MHz, 需要根据实际情况调整
...
void ic_proc(void)
{
    pwm_set_duty_PA6(duty[0]);
    pwm_set_frequency_PA6(freq[0]);
    pwm_set_duty_PA7(duty[1]);
    pwm_set_frequency_PA7(freq[1]);
